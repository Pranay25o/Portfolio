...
/*
 * PERFORMANCE FIX: Removed ALL Framer Motion usage from this component.
 *
 * Before: 6x motion.button (deck cards) + 3x motion.div (achievement pills) +
 *         2x motion.button (CTA buttons) = 11 Framer Motion-managed elements.
 *         Every hover triggered Framer Motion's JS animation engine to calculate
 *         and apply transforms per frame, competing with the main thread.
 *
 * After: Pure CSS transitions on all interactive elements. The browser handles
 *        transform/opacity animations entirely on the compositor thread —
 *        zero JS execution during hover/interaction.
 *
 * The mount animation on the main card (opacity: 0 → 1, scale: 0.98 → 1) is
 * replaced with a CSS animation class for the same reason.
 *
 * Visual output is IDENTICAL — same values, same easing, same durations.
 */
...
export const HeroCardDeck: React.FC<HeroCardDeckProps> = ({
  ...
  return (
    <div className="relative w-full max-w-7xl mx-auto pt-20 pb-8 px-2 sm:px-4 lg:px-6 flex flex-col items-center">
      ...
      {/* 3D Arc of Theme Cards in Background — pure CSS hover transitions */}
          ...
          {deckCards.map((card, index) => {
            ...
            return (
              <button
                ...
                /*
                 * CSS hover replaces motion.button whileHover={{ y: -18, scale: 1.06, zIndex: 30 }}
                 * The deck-card class below handles the transform on :hover via CSS.
                 */
                className={`deck-card group relative w-24 sm:w-36 md:w-44 h-24 sm:h-32 md:h-40 rounded-t-2xl sm:rounded-t-3xl p-2.5 sm:p-3.5 border-t border-l border-r bg-gradient-to-b ${card.styleClass} backdrop-blur-xl shadow-2xl flex flex-col justify-between overflow-hidden cursor-pointer transition-all duration-300 ${
                  isFront
      ...
      {/* 🌟 FRONT & CENTER MASTERPIECE GLASS CARD
       * One-time mount animation kept as CSS animation (hero-card-mount class).
       * No JS involved — pure CSS @keyframes on entry.
       */}
      <div
        className="hero-card-mount relative w-full max-w-5xl rounded-3xl sm:rounded-[36px] overflow-hidden border border-orange-500/40 border-t-amber-400/70 border-b-black/90 shadow-[0_20px_80px_rgba(0,0,0,0.9),0_0_40px_rgba(234,88,12,0.35)] backdrop-blur-xl z-20"
        style={{
        ...
        <div className="relative z-10 p-6 sm:p-10 md:p-12 flex flex-col justify-between min-h-[480px] sm:min-h-[520px] gap-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-5 text-left">
              ...
              {/* Location & Social Proof Links Row */}
             ...
             * CSS hover replaces motion.div whileHover={{ x: 6, skewX: -2 }}
             * using the achievement-pill class defined in index.css
             */}
            <div className="lg:col-span-5 flex flex-col gap-3.5 w-full">

              {/* Pill 1: GATE 2026 */}
              <div className="achievement-pill group relative w-full p-4 rounded-2xl bg-black/55 hover:bg-orange-950/40 border border-white/10 hover:border-orange-500/50 backdrop-blur-md shadow-lg cursor-pointer transition-all">
          ...
          <div className="pt-4 border-t border-white/10 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full">
              ...
              <button
                ...
                className="cta-button w-full sm:w-auto px-8 py-3.5 rounded-full font-mono text-sm font-medium tracking-wider text-slate-200 bg-white/5 hover:bg-white/10 border border-white/20 hover:border-white/40 backdrop-blur-md transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
